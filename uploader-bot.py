		...
		class BotManager:
			...
			def generate_link(self, category_id: str) -> str:
				...
				if self.bot_username:
					return f
					"https://t.me/{self.bot_username}?start=cat_{category_id}"
				# Fallback در صورت عدم وجود یوزرنیم
				bot_id = BOT_TOKEN.split(':')[0]
				return f
				"https://t.me/{bot_id}?start=cat_{category_id}"
			...
			def extract_file_info(self, update: Update) -> dict:
				...
				if msg.document:
					file = msg.document
					file_type = 'document'
					file_name = file.file_name or f
					"document_{file.file_id[:8]}"
				elif msg.photo:
					file = msg.photo[-1]  # بالاترین کیفیت
					file_type = 'photo'
					file_name = f
					"photo_{file.file_id[:8]}.jpg"
				elif msg.video:
					file = msg.video
					file_type = 'video'
					file_name = f
					"video_{file.file_id[:8]}.mp4"
				elif msg.audio:
					file = msg.audio
					file_type = 'audio'
					file_name = f
					"audio_{file.file_id[:8]}.mp3"
				else:
					return None
				...
				async def handle_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
					...
					async def handle_timer_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
						...
						if seconds == -1:
							...
							status = f
							"✅ تایمر اختصاصی تنظیم شد به: {seconds} ثانیه" if seconds > 0 else "✅ تایمر غیرفعال شد"
							await update.message.reply_text(status)
							...
							async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
								...
								if data.startswith('check_'):
									...
									if non_joined:
										...
										# حالا عضو شده است
										await query.edit_message_text(
											"✅ عضویت شما تأیید شد! در حال آماده‌سازی فایل‌ها...")
										...
										async with aiohttp.ClientSession() as session:
											async with session.get("https://final-bot-d3dk.onrender.com") as resp:
												if resp.status == 200:
													...
													logger.warning(f
													"⚠️ Keep-alive failed: {resp.status}")
										except Exception as e:
										logger.warning(f
										"⚠️ Keep-alive exception: {e}")
